...
final class SecurePinEnrollmentStore implements PinEnrollmentStore {
  ...
  @override
  Future<bool> get isEnrolled async => (await _storage.read(key: _recordKey)) != null;

  @override
  Future<void> enroll(String pin) async {
    final salt = Uint8List.fromList(
      List<int>.generate(_saltBytes, (_) => Random.secure().nextInt(256)),
    );
  ...
  Future<bool> verify(String candidate) async {
    ...
    try {
      ...
      final salt = base64Decode(record['s'] as String);
      final expected = base64Decode(record['d'] as String);
      final actual = await _derive(candidate, salt);
